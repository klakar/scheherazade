...
class story:
    ...
    def slide_add(self):
    	...
    	self.save_current_slide
    	currentSlide += 1
    	slideTitle.insert(currentSlide, self.tr("Title"))
    	slideContent.insert(currentSlide, self.tr("Content"))
    	slideZoom.insert(currentSlide, 18)
    	slidePosition.insert(currentSlide, "15,65")
    	#self.open_current_slide()
    	#QgsMessageLog.logMessage("Current slide is: %s " % (currentSlide), 'Story', QgsMessageLog.INFO)
    ...
    def open_current_slide(self):
    	global currentSlide
    	QgsMessageLog.logMessage("Current slide count is: %s " % (len(slideTitle)), 'Story', QgsMessageLog.INFO)
    	self.dockwidget.txtSlideTitle.setText(slideTitle[currentSlide])
    	self.dockwidget.txtSlideContent.setText(slideContent[currentSlide])
    	#self.dockwidget.lblSlideNumber.setText(self.tr('Slide %s/%s') % (currentSlide, slideTitle.count()))
